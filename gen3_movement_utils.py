...
class Arm:
    def __init__(self):
        ...
        try:
            ...
            self.degrees_of_freedom = rospy.get_param(
                "/" + self.robot_name + "/degrees_of_freedom", 6)
            self.is_gripper_present = rospy.get_param(
                "/" + self.robot_name + "/is_gripper_present", True)
            ...
            self.stop_publisher = rospy.Publisher(
                "/my_gen3_lite/in/stop", std_msgs.msg.Empty, queue_size=1, latch=True)
            ...
            rospy.wait_for_service("/my_gen3_lite/base/play_cartesian_trajectory")
    ...
    def get_fk(self, joints=None):
        ...
        joints: list or dictionary of joints, 
            lists are assumed to be in order of joint1-5/6
        """

        if joints is None:
            joint_state = rospy.wait_for_message(
                "/my_gen3_lite/joint_states", JointState)
        else:
        ...
        compute_fk = rospy.ServiceProxy("/my_gen3_lite/compute_fk", GetPositionFK)
    ...
    def get_joints(self, seven_dof=True):
        """
        Returns current joints as a list in order of joints 1-6
        """
        ...
        if seven_dof:
            ...
            return joint_states.position[0:5]
    ...
    def goto_cartesian_pose(self, pose, relative=False, check_collision=False, wait_for_end=True,
        ...
        if isinstance(pose, (list, np.ndarray)):
            if radians is False:
                ...
                euler_corr = euler_from_quaternion((pose.pose.orientation.x, pose.pose.orientation.y, pose.pose.orientation.z))
            else:
                temp_pose = PoseStamped()
                temp_pose.pose.position.x = pose[0]
                temp_pose.pose.position.y = pose[1]
    ...
    def goto_joint_pose(self, joints, radians=False):
        ...
        joints: list of joint anlges (from 1 to 6)
    ...
    def joint_velocity_command(self, values, duration, duration_timeout=None, collision_check=False):
        ...
        If it is a list or dictionary, it is assumed to be ordered 1-6.
        ...
        def velocity_command(values, duration):
            joint_vel_publisher = rospy.Publisher(
                "/my_gen3_lite/in/joint_velocity", Base_JointSpeeds, queue_size=10, latch=True)
            stop_publisher = rospy.Publisher(
                "/my_gen3_lite/in/stop", std_msgs.msg.Empty, queue_size=10, latch=True)
    ...
    def cartesian_velocity_command(self, values, duration, duration_timeout=None, collision_check=False, radians=False):
        ...
        def velocity_command(values, duration):
            cartesian_vel_publisher = rospy.Publisher(
                "/my_gen3_lite/in/cartesian_velocity", TwistCommand, queue_size=1, latch=True)
            #stop_publisher = rospy.Publisher(
            #    "/my_gen3_lite/in/stop", std_msgs.msg.Empty, queue_size=1, latch=True)
            empty_message = std_msgs.msg.Empty()
    ...
    def goto_joint_pose_sim(self, joints):
        ...
        --------
        joints: list or np array of joint anlges (from 1 to 6) 
        """

        go_to_joints = rospy.ServiceProxy(
            "/my_gen3_lite/base/play_joint_trajectory", PlayJointTrajectory)
    ...
    def goto_cartesian_pose_sim(self, pose, speed=.1, duration=None, radians=True):
        ...
        go_to_cart = rospy.ServiceProxy(
            "/my_gen3_lite/base/play_cartesian_trajectory", PlayCartesianTrajectory)
